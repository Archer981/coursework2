...
def index():
    return render_template('index.html', posts=posts.load_all(),
...
def post_page(pk):
    post = posts.load_post(pk)
    comments_loaded = comments.load_comments(pk)
    if not post:
        return redirect('/')
    comments.counter.setdefault(pk, 0)
    bookmark_status = bookmarks.get_bookmark(pk)
    return render_template('post.html',
                           post=post, comments=comments_loaded,
...
def search_page():
    ...
    posts_found = posts.find_posts(substr)
    if not posts_found:
        return redirect('/')
